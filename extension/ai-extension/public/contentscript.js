 ...
 * @param {Function} func The function to d  const headerTitle = document.createElement('h2');
  headerTitle.id = 'sentry-alert-title';
  ...
  bubbleHeader.appendChild(headerTitle);
  
  // Add explanation text
  const explanation = document.createElement('p');
  explanation.id = 'sentry-alert-description';
  explanation.style.cssText = `
    ...
    padding-right: 10px;
    font-weight: 500;
  `;{number} delay The debounce delay in milliseconds.
...
async function scanPageWithSentryAI() {
  ...
  try {
    ...
    const hasObviousVulgarContent = vulgarRegex.test(contentToScan);
...
function applySentryAction(ai) {
  // Get keywords from AI response to identify problematic content
  // We'll use these to identify content to blur
  const keywords = ai.bad_words || [];
  const category = ai.category || "unsafe content";
  
  // Check if this is a scam detection
  const isScamDetection = category.includes('scam') || 
                          keywords.some(word => ['scam', 'phishing', 'suspicious'].includes(word));
  
  // For scam detection, we need to take special actions
  if (isScamDetection && isMessenger) {
    // For messenger scams, try to find the entire message container
    const messageContainers = document.querySelectorAll('.x78zum5, .xcrwhx7, [role="row"], .message-container');
    
    // Apply immediate blur to the entire message that contains scam content
    messageContainers.forEach(container => {
      // Check if this container has suspicious content
      let containerText = container.innerText || '';
      
      // Look for scam indicators in this specific container
      ...
      if (hasScamIndicators) {
        console.log("Sentry: Blurring entire scam message container");
  ...
  // Detect problematic elements based on keywords (standard approach)
  const textNodes = findTextNodesToBlur(keywords);
  const imageNodes = isScamDetection ? [] : findImagesToBlur(); // Only blur images for non-scam content
  
  // Track if we've applied any blur effects
  let hasAppliedBlur = false;
  
  // Apply blur effect to problematic text
  textNodes.forEach(nodeInfo => {
    const { node, parent } = nodeInfo;
    
    // Skip if node or parent is missing (sometimes happens with dynamic content)
    if (!node || !parent) return;
    
    // Skip if parent no longer in DOM
    if (!document.body.contains(parent)) return;
    
    // Create a wrapper for the text node if it doesn't already have one
    const wrapper = document.createElement('span');
    wrapper.classList.add('sentry-blurred-content');
    ...
    // Replace the text node with our wrapper containing the text
    ...
    try {
      ...
      // Add event listeners to allow toggling blur on click
      wrapper.addEventListener('click', (e) => {
        e.stopPropagation();
        wrapper.style.filter = wrapper.style.filter === 'none' ? 'blur(5px)' : 'none';
      });
      
      hasAppliedBlur = true;
    } catch (err) {
      console.error("Sentry: Error applying blur to text node", err);
    }
  });
  
  // Apply blur effect to problematic images
  imageNodes.forEach(img => {
    // Skip if image is no longer in DOM
    if (!document.body.contains(img)) return;
    
    img.classList.add('sentry-blurred-content');
    img.style.cssText = `
      filter: blur(10px);
      transition: filter 0.3s ease;
    `;
    ...
    try {
      ...
      imgOverlay.appendChild(warningIcon);
      ...
      if (img.parentElement) {
        img.parentElement.appendChild(imgOverlay);
        
        // Add event listeners to allow toggling blur on click
        imgOverlay.addEventListener('click', (e) => {
          e.stopPropagation();
          img.style.filter = img.style.filter === 'none' ? 'blur(10px)' : 'none';
        });
        
        hasAppliedBlur = true;
      }
    } catch (err) {
      console.error("Sentry: Error applying blur to image", err);
    }
  });
  
  // Create the notification - always show notification if content is detected,
  // even if blurring couldn't be applied (this ensures user is aware)
  if (hasAppliedBlur || keywords.length > 0) {
    createNotificationBubble(ai, category);
  }
}
